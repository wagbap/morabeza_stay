...
const MeusExperiencias = () => {
  ...
  const carregarExperiencias = async (uid) => {
    ...
    try {
      setLoading(true);
      const result = await listarExperiencias(uid);

      if (result.success) {
        setExperiencias(result.data?.items || []);
      } else {
        setError(result.message || 'Erro ao carregar experiências');
      }
    } catch (err) {
      console.error('[MeusExperiencias] Erro:', err);
  ...
  useEffect(() => {
    ...
    setUsuarioId(uid);
    carregarExperiencias(uid);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleEditar = (id) => navigate(`/experiencia-registo/editar/${id}`);
  const handleVer = (id) => navigate(`/experiencia-registo/detalhes/${id}`);
  ...
  const handleExcluir = async (id, titulo) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast('Experiência excluída com sucesso!', 'success');
      } else {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center max-w-md">
          ...
          <button
            onClick={() => usuarioId && carregarExperiencias(usuarioId)}
          ...
          <div className="flex justify-between items-center flex-wrap gap-4">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">
                Gerencie todas as suas experiências cadastradas
              </p>
            </div>
            <button
              ...
              >
                <div className="relative h-48 bg-gray-100">
                  {exp.imagem_principal ? (
                    <img
                      src={
                        exp.imagem_principal.startsWith('http')
                          ? exp.imagem_principal
                          : `${API_URL}${exp.imagem_principal}`
                      }
                      alt={exp.titulo}
                  ...
                  <div className="absolute top-3 right-3">{getStatusBadge(exp.status)}</div>
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-3 text-sm text-gray-500 mb-3">
                    <span className="flex items-center gap-1">
                      <MapPin size={14} /> {exp.localizacao || exp.ilha}
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock size={14} /> {exp.duracao}
                    </span>
                    <span className="flex items-center gap-1">
                      <Users size={14} /> até {exp.max_pessoas} pessoas
                    </span>
                  </div>
