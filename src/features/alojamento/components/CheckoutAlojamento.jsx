// CheckoutAlojamento.jsx - Versão completamente funcional
import React, { useState, useEffect } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import { 
  Check, ArrowLeft, Loader, AlertCircle, ChevronRight, 
  Calendar, Users, Home, CreditCard, Lock, ShieldCheck
} from 'lucide-react';
  ...
  return (
    <div className="mb-8 p-6 bg-white border border-slate-200 rounded-2xl shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-xs font-bold text-slate-700 block mb-1">Nacionalidade</label>
          <select 
      ...
      {participantes.length === 0 ? (
        ...
        <div className="space-y-4">
          {participantes.map((p, idx) => (
            <div key={p.id} className="border border-slate-100 rounded-xl p-4 relative">
              <button 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <input 
                  ...
                  className="border border-slate-200 rounded-xl p-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-600/20"
                />
                <select 
                  value={p.idade}
...
const ResumoReservaAlojamento = ({ reserva, totalHospedes, precoTotal, showPaymentInfo = false, paymentStatus = null }) => {
    ...
    <div className="border border-slate-200 rounded-2xl p-5 bg-white shadow-sm sticky top-6">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-slate-500 mt-1">{reserva?.localizacao || 'Cabo Verde'}</p>
        </div>
      </div>
      ...
      <div className="space-y-4 border-t border-slate-100 pt-5">
        ...
        <div className="flex justify-between">
          <span className="text-xs text-slate-600">Noites</span>
          <span className="text-xs font-bold text-blue-900">{reserva?.noites || 0} noites</span>
        </div>
        <div className="flex justify-between">
          <span className="text-xs text-slate-600">Hóspedes</span>
        ...
        <div className="pt-3 space-y-2 border-t border-slate-100">
          <div className="flex justify-between text-[11px]">
            ...
            <span>{formatNumber(reserva?.precoNoite)} CVE</span>
          </div>
          <div className="flex justify-between text-[11px]">
            <span className="text-slate-500">Subtotal ({reserva?.noites || 0} noites)</span>
            <span>{formatNumber(reserva?.subtotal)} CVE</span>
          </div>
          <div className="flex justify-between text-[11px]">
            <span className="text-slate-500">Taxa de limpeza</span>
            <span>{formatNumber(reserva?.taxaLimpeza)} CVE</span>
          </div>
          <div className="flex justify-between text-[11px]">
            <span className="text-slate-500">Taxa de serviço</span>
            <span>{formatNumber(reserva?.taxaServico)} CVE</span>
          </div>
        </div>
        ...
        <div className="bg-green-50 p-3 rounded-xl flex gap-2 mt-3 border border-green-100">
          ...
          <div>
            ...
            <p className="text-[8px] text-green-700">Até 48 horas antes do check-in</p>
          </div>
        </div>

        <div className="bg-[#F0F7FF] p-3 rounded-xl flex gap-2">
          <Lock className="text-blue-600 shrink-0" size={16} />
          <p className="text-[8px] text-blue-700">Seus dados estão protegidos e seguros</p>
        </div>
      </div>
    </div>
  );
