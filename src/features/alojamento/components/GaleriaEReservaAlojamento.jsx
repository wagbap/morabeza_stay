...
const GaleriaEReserva = ({ alojamento }) => {
  ...
  const hotel = {
    titulo: alojamento?.titulo || t('alojamento_maravilha'),
    localizacao: alojamento?.localizacao || t('cabo_verde'),
  ...
  // Fotos de exemplo para preencher a galeria
  const fotosGaleria = [
    hotel.fotoPrincipal,
  ...
  const handleReserva = () => {
    setLoading(true);
    alert(t('redirecionar_pagamento'));
      ...
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            <span className="bg-[#003580] text-white text-[10px] font-bold px-2 py-1 rounded-sm uppercase tracking-wider">
              {t('genius')}
            </span>
          ...
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-gray-600 mt-2">
            ...
            <span className="text-[#006ce4] font-bold cursor-pointer hover:text-blue-800">
              {t('excelente_localizacao')} — {t('mostrar_mapa')}
            </span>
          </div>
        </div>

        {/* --- AÇÕES E BOTÃO RESERVAR --- */}
        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
          ...
          >
            {loading ? t('a_processar') : t('reserve_agora')}
          </button>
        </div>
      </div>
