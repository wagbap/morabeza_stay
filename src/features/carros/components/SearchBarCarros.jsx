// src/features/carro/components/SearchBarCarros.jsx
...
const LOCAIS = [
  ...
  { value: 'Praia',       labelKey: 'cidade_praia',           fallback: 'Praia' },
  { value: 'Aeroporto',   labelKey: 'aeroporto_nelson_mandela', fallback: 'Aeroporto Nelson Mandela' },
...
// ------------------------------------------------------------
const SearchBarCarros = ({ onBuscar, className = '' }) => {
  ...
  const [startDate, setStartDate] = useState(fromISODate(urlParams.get('data_levantamento')));
  const [endDate,   setEndDate]   = useState(fromISODate(urlParams.get('data_devolucao')));
  ...
  useEffect(() => {
    ...
    setStartDate(fromISODate(urlParams.get('data_levantamento')));
    setEndDate(fromISODate(urlParams.get('data_devolucao')));
  }, [urlParams]);
  ...
  const handleSearch = useCallback((e) => {
    ...
    const params = {
      localizacao: destino,
      data_levantamento: toISODate(startDate),
      data_devolucao: toISODate(endDate),
    };
      ...
      >
          ...
          <div className="flex items-center gap-3 w-full text-left">
            ...
            <div className="flex flex-col flex-1 min-w-0">
              <label className="text-[10px] uppercase font-black text-gray-400 tracking-wider">
                {t('local_levantamento', 'LOCAL DE LEVANTAMENTO')}
              </label>
              <div className="relative flex items-center">
                <select
                  value={destino}
                  onChange={(e) => setDestino(e.target.value)}
                  aria-label={t('local_levantamento', 'Local de levantamento')}
        ...
        <div ref={calendarioRef} className="flex-[1.4] relative flex items-center px-4 py-3 border-b md:border-b-0 md:border-r border-gray-100 hover:bg-gray-50 transition-colors rounded-xl">
          ...
          >
            ...
            <div className="flex flex-col flex-1 min-w-0">
              <label className="text-[10px] uppercase font-black text-gray-400 tracking-wider">
                {t('levantamento_devolucao', 'LEVANTAMENTO — DEVOLUÇÃO')}
              </label>
        ...
        <div className="w-full md:w-auto flex gap-2">
          ...
          >
            ...
            <span>{t('buscar_veiculo', 'BUSCAR')}</span>
          </button>
        </div>
      </form>
