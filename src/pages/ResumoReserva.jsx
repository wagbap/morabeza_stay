...
const ResumoReserva = ({ 
  ...
  return (
    <div className="border border-slate-200 rounded-2xl p-5 bg-white shadow-sm sticky top-6">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 space-y-1 text-[10px] font-semibold text-slate-400">
            <p className="flex items-center gap-1.5"><Clock size={11}/> {reserva.duracao}</p>
            <p className="flex items-center gap-1.5"><Users size={11}/> 1 - {reserva.maxPessoas} pessoas</p>
            <p className="flex items-center gap-1.5"><MapPin size={11}/> {reserva.localizacao}</p>
          </div>
        </div>
      </div>
      ...
      {/* Status de pagamento - com design melhorado */}
      ...
      <div className="space-y-4 border-t border-slate-100 pt-5">
        {/* Data selecionada - botão alterar condicional */}
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">{reserva.data}</p>
          </div>
        ...
        {/* Período - botão alterar condicional */}
        <div className="flex justify-between items-start">
          <div>
            <p className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">{reserva.periodo} ({reserva.horario})</p>
          </div>
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-500 text-[11px] mt-0.5">{totalPessoas} {totalPessoas === 1 ? 'pessoa' : 'pessoas'}</p>
          </div>
          {!podeAlterar && (
            <span className="text-green-600 text-[9px] font-bold bg-green-50 px-2 py-0.5 rounded-full">
              Confirmado
        ...
        <div className="pt-3 space-y-1">
          <div className="flex justify-between text-[11px]">
            ...
            <span className="font-bold text-blue-900">{reserva.precoPorPessoa?.toLocaleString()} CVE</span>
          </div>
          <div className="flex justify-between text-[11px]">
            <span className="text-slate-500">Total ({totalPessoas} pessoas)</span>
            <span className="font-bold text-blue-900">{precoTotal.toLocaleString()} CVE</span>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center pt-3 border-t border-slate-100">
          ...
          <div className="text-right">
            <span className="text-xl font-bold text-blue-600">{precoTotal.toLocaleString()} CVE</span>
            {paymentStatus === 'paid' && (
