...
const ExperienciaDetalhes = () => {
  ...
  // 🔥 NOVO: Buscar usuário logado do localStorage
  useEffect(() => {
  ...
  const nextImage = () => {
    if (experiencia?.imagens && experiencia.imagens.length > 0) {
      const newIndex = (imagemIndex + 1) % experiencia.imagens.length;
      setImagemIndex(newIndex);
      setImagemPrincipal(experiencia.imagens[newIndex].caminho_url);
  ...
  const prevImage = () => {
    if (experiencia?.imagens && experiencia.imagens.length > 0) {
      const newIndex = (imagemIndex - 1 + experiencia.imagens.length) % experiencia.imagens.length;
      setImagemIndex(newIndex);
      setImagemPrincipal(experiencia.imagens[newIndex].caminho_url);
      setImageLoading(true);
    }
    ...
    return (
      <div className="bg-[#fcfcfc] min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-bold text-[#1a2b6d]">Carregando detalhes da experiência...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="bg-[#fcfcfc] min-h-screen flex items-center justify-center">
        <div className="text-center p-8">
          <p className="font-bold text-red-500 text-xl mb-4">{error || "Experiência não encontrada."}</p>
          <button 
  ...
  // Data mínima (hoje) e máxima (6 meses)
  const minDate = new Date().toISOString().split('T')[0];
  const maxDate = new Date();
  maxDate.setMonth(maxDate.getMonth() + 6);
  const maxDateStr = maxDate.toISOString().split('T')[0];

  // 🔥 Função para abrir modal de login (opcional)
  const handleOpenLoginModal = () => {
    alert('Por favor, clique no botão "Entrar com Google" no canto superior direito da tela para fazer login.');
  };
  ...
  return (
    <div className="bg-[#fcfcfc] min-h-screen pb-20 font-sans text-[#1a2b6d]">
      <nav className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-2 text-[11px] font-medium text-slate-500">
        ...
        <span className="cursor-pointer hover:text-blue-600" onClick={() => navigate('/experiencias')}>Experiências</span> 
        <ChevronRight size={10} />
          ...
          <div className="lg:col-span-2">
            <div className="space-y-3 mb-6">
              <div className="relative h-[300px] md:h-[380px] rounded-2xl overflow-hidden group bg-slate-100">
                ...
                />
                {experiencia.imagens && experiencia.imagens.length > 1 && (
                  <>
                    <button 
                      onClick={prevImage}
              ...
              {experiencia.imagens && experiencia.imagens.length > 0 && (
                <div className="grid grid-cols-5 gap-2 h-20">
                  {experiencia.imagens.slice(0, 4).map((img, i) => (
                    <div 
                      key={i} 
                      onClick={() => trocarImagemPrincipal(img.caminho_url, i)}
                    ...
                    >
                      <img src={img.caminho_url} className="w-full h-full object-cover" alt={`Thumbnail ${i + 1}`} />
                    </div>
                  ))}
                  {experiencia.imagens.length > 4 && (
                    <div className="bg-slate-50 border border-slate-200 rounded-xl flex flex-col items-center justify-center gap-0.5 text-slate-500 cursor-pointer hover:bg-slate-100 transition-colors">
                      <Camera size={16}/>
                      <span className="text-[9px] font-black uppercase tracking-tighter">+{experiencia.imagens.length - 4}</span>
                    </div>
                  )}
                </div>
              )}
  ...
  {experiencia.rating_formatado || '5.0'} ({experiencia.total_avaliacoes || 0} avaliações)
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {periodosUI.map((p) => {
                      ...
                      return (
                        ...
                        >
                          <p.Icon size={18} className={isSelected ? p.color : 'text-slate-300'} />
        ...
        {/* 🔥 SEÇÃO DE AVALIAÇÕES - PASSANDO O USUÁRIO LOGADO */}
        <div className="mt-16 border-t border-slate-100 pt-12">
          <AvaliacoesSeccao 
            experienciaId={experiencia?.id || 1} 
